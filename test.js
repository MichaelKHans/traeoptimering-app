        ...
        window.autoFillStock = function() {
            ...
            // Brug den avancerede analyse til at finde optimale bestillingslængder
            const analysis = runAdvancedAnalysis(globalMargin);
            ...
            if (hasPurchases) {
                ...
                alert("Kunne ikke finde nogle optimale bestillingslængder.");
            }
        };
        ...
        function handleAddStock(e) {
            ...
            const stock = { id: nextStockId++, dimension: lastDim, length: parseInt(lastLen), amount: parseInt(fd.get('amount')) };
        ...
        function runOptimization(globalMargin, currentStock, piecesToProcess) {
            ...
            piecesToProcess.forEach(p => {
                ...
                piecesByDim[p.dimension].push({ id: p.id, exactLength: p.length, cutLength: p.length + margin, margin: margin, tag: p.tag });
            });

            const stockByDim = {};
            currentStock.forEach(s => {
                if (!stockByDim[s.dimension]) stockByDim[s.dimension] = [];
                for (let i = 0; i < s.amount; i++) { stockByDim[s.dimension].push(s.length); }
            });
            ...
            for (const dim in piecesByDim) {
                ...
                if (currentStock.length > 0 && available.length === 0) throw new Error(`Mangler lager til ${dim}`);
                ...
                for (const piece of pieces) {
                    ...
                    for (const s of used) {
                        if (s.rem >= piece.cutLength) { s.rem -= piece.cutLength; s.pieces.push(piece); placed = true; break; }
                    }
                    if (!placed) {
                        available.sort((a, b) => a - b);
                        const idx = available.findIndex(l => l >= piece.cutLength);
                        if (idx === -1) throw new Error(`Ikke nok træ (${dim}) til ${piece.exactLength}mm + margin`);
                        const len = available.splice(idx, 1)[0];
                        used.push({ originalLength: len, rem: len - piece.cutLength, pieces: [piece] });
                    }
                }
                finalPlan[dim] = used.map(s => { usedStockLen += s.originalLength; return { originalLength: s.originalLength, pieces: s.pieces, waste: s.rem }; });
            }
            return { plan: finalPlan, totalUsedStockLength: usedStockLen, totalRequiredLength: reqLen, totalWaste: usedStockLen - reqLen, wastePercentage: usedStockLen > 0 ? ((usedStockLen - reqLen) / usedStockLen) * 100 : 0 };
        }
        ...
        function renderStockList() {
            stockListContainer.innerHTML = stockLengths.map(s => `
                <div style="display:flex;align-items:center;gap:6px;padding:6px 10px;background:#f8fafc;border-radius:8px;border:1px solid #e2e8f0;border-left:3px solid #94a3b8;flex-wrap:nowrap;overflow:hidden">
                    ...
                    <span style="color:#94a3b8;font-size:0.75rem;flex-shrink:0">mm</span>
                    <button onclick="removeStock(${s.id})" style="margin-left:auto;flex-shrink:0;color:#94a3b8;font-size:0.8rem;font-weight:600;background:none;border:none;cursor:pointer;white-space:nowrap">Fjern</button>
                </div>
        ...
        function renderResults(res) {
            ...
            for (const dim of sortedDimensions) {
                ...
                dimPlan.forEach(stock => { totalPiecesInDim += stock.pieces.length; });
                ...
                <div class="mt-6 border border-gray-200 rounded-xl overflow-hidden">
                    <div class="bg-gray-100 px-4 py-3 border-b border-gray-200">
                        ...
                        <p class="text-sm text-gray-600 mt-0.5">
                            <span class="font-semibold text-gray-800">${stockCount} stk. lager-/bestillingslængde${stockCount !== 1 ? 'r' : ''}</span> ${lengthLabel}
                ...
                dimPlan.forEach((stock, i) => {
                    ...
                    planHTML += `<div class="mt-3 stock-block">
                        <div class="flex items-baseline gap-2 mb-1">
                            <span class="text-xs font-bold text-slate-700">Længde ${i + 1} af ${stockCount}</span>
        ...
        function handleLoadFromFile(e) {
            ...
            reader.onload = function (evt) {
                try {
                    ...
                    if (!data.availableDimensions || !data.requiredPieces) throw new Error('Ugyldigt filformat.');
                    availableDimensions = data.availableDimensions || [];
                    requiredPieces = data.requiredPieces || [];
                    stockLengths = data.stockLengths || [];
                    safetyMarginInput.value = data.safetyMargin || 100;
        ...
        function renderSustainabilityReport(analysis, plan) {
            ...
            if (hasPurchases) {
                ...
                applyBtnHtml = `<div class="mt-2"><button onclick="window.applyOptimalPurchase('${encodedSuggestions}')" class="text-xs text-slate-500 hover:text-emerald-600 underline underline-offset-2 transition-colors cursor-pointer focus:outline-none">Erstat lager med disse længder</button></div>`;
            }
        ...
        function populateProjectsDropdown() {
            ...
            if (projectNames.length === 0) {
                savedProjectsSelect.innerHTML = `<option>Ingen gemte projekter</option>`;
                savedProjectsSelect.disabled = true; loadProjectBtn.disabled = true; deleteProjectBtn.disabled = true;
            } else {
                savedProjectsSelect.innerHTML = projectNames.map(name => `<option value="${name}">${name}</option>`).join('');
